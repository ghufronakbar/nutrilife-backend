...
class TrackController extends BaseController_1.BaseController {
    constructor() {
        super();
        this.getProgress = async (req, res) => {
            ...
            try {
                ...
                const foodLogs = await db_1.default.foodLog.findMany({
                    where: {
                        userPreferences: {
                            userId: userId,
                        },
                ...
                // Hitung streak
                let streak = 0;
                let current = (0, moment_timezone_1.default)().tz("Asia/Jakarta").startOf("day");
                while (true) {
                ...
                const caloriesTracked = foodLogs.reduce((acc, log) => acc + log.calories, 0);
                const data = {
                    dayStreak: streak,
                    dayTracked,
                    caloriesTracked,
        ...
        this.getAchievement = async (req, res) => {
            var _a;
            try {
                const userId = (_a = req === null || req === void 0 ? void 0 : req.user) === null || _a === void 0 ? void 0 : _a.id;
                ...
                const dailyMap = {};
                foodLogs.forEach((log) => {
                    const dateStr = (0, moment_timezone_1.default)(log.createdAt)
                        .tz("Asia/Jakarta")
                        .format("YYYY-MM-DD");
                    if (!dailyMap[dateStr]) {
                        dailyMap[dateStr] = {
                ...
                const proteinDays = dates.filter((d) => dailyMap[d].protein >= proteinGoal);
