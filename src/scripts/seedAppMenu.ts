...
export async function seedAppMenu() {
  ...
  for await (const menu of menus) {
    ...
    const plans = await db.appPlanningMenu.findMany({
      where: {
        type: menu.type as $Enums.MealType,
      },
    });

    for await (const plan of plans) {
      const connectPlan = await db.appMenu.update({
        where: {
          id: menuCreated.id,
        },
        data: {
      ...
      console.log(
        `✔️  Ditambahkan: ${connectPlan.name} ke hari ${connectPlan.appPlaningMenus[0].day} ${connectPlan.appPlaningMenus[0].type}`
      );
    }
  }
}
